...
public class GlobalPermissionsTable extends Table{

	public GlobalPermissionsTable(SQLite db) {
		super(db, "global_permissions", Arrays.asList("permission"));
	...
	public Permission getPermission(String permission) {
		...
		try {
			...
			if (rs.next()) {
				perm = new Permission(rs.getString("permission"), rs.getInt("check_order"), rs.getInt("num_storage"));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
	...
	public void setPermission(Permission permission) {
		...
		if(hasRecord) {
			...
			getDb().getPlugin().getLogger().log(Level.WARNING, "Missing possible record with permission: " + permission);
		}
		return;
	}
	...
	public void addPermission(Permission permission) {
		...
		try {
			...
			ps = conn.prepareStatement("INSERT INTO " + this.getName() + 
					" (permission, num_storage, check_order)"
					+ " VALUES (?, ?, ?)");
