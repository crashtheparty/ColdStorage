...
public class ChestTable extends Table {
	...
	public List<Chest> getChests(OfflinePlayer player) {
		...
		try {
			...
			ps = conn.prepareStatement("SELECT * FROM " + getName() + " WHERE player_unique = '" + player.getUniqueId().toString() + "';");
	...
	public Chest getChest(String unique) {
		...
		try {
			conn = getDb().getSQLConnection();
			ps = conn.prepareStatement("SELECT * FROM " + getName() + " WHERE `unique` = '" + unique + "';");
	...
	public Chest getChest(Location loc) {
		...
		try {
			conn = getDb().getSQLConnection();
			ps = conn.prepareStatement("SELECT * FROM " + getName() + " WHERE location_one = '" + location + "' OR location_two = '" + location + "';");
	...
	public boolean addChest(Chest chest) {
		...
		try {
			conn = getDb().getSQLConnection();
			ps = conn.prepareStatement("INSERT INTO " + this.getName() + " (`unique`, player_unique, location_one, location_two)" + " VALUES (?, ?, ?, ?)");
	...
	public boolean deleteChest(Chest chest) {
		...
		try {
			conn = getDb().getSQLConnection();
			ps = conn.prepareStatement("DELETE FROM " + this.getName() + " WHERE `unique` = ?");
